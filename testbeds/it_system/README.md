...
gateway, with `n_1` doubling as a management host. CCD's nominal-operation dataset `D`
is *measured* on the running containers (replacing the simulator), then used to select
and enact degraded modes and to validate the estimated functionality against reality.
...
- **gateway** (`ccd_gateway`): async round-robin reverse proxy, *no health checks / no
  skip-on-failure* so the per-server offered load `L_i ~ W/m` even when a link is closed.
  Counters `attempted[i]` (= `L_i`) and `ok[i]` (= `Th_i`); `/metrics`, `/work`.
- **server i** (`ccd_server{i}`, hostname `n{i}`): Flask + gunicorn (1 worker), `/work`
  does an upsert+read against postgres. Counters `requests_completed_db` (= `Tt_i`),
  etc.; `/metrics` published on host port `5000+i` (bypasses the blockable links).
- **db** (`ccd_db`): postgres:16, `pg_isready` healthcheck, ephemeral.
...
blocks the management link `n_1`→`n_i` (at server 1). Closures use `REJECT --reject-with
tcp-reset` (fail-fast, so `L_i` is preserved and toggles take effect immediately). The
same mechanism drives nominal maintenance toggling during collection and enacts CCD's mode.
...
python scripts/generate_dataset.py --m 3 --quick --out data/smoke.csv   # ~5 min
python scripts/run_ccd.py --data data/smoke.csv --m 3          # -> data/ccd_result.json
...
python scripts/run_ccd.py --data data/smoke.csv --m 3 --patched   # D_2 -> do(N1=0)
python scripts/run_ccd.py --data data/smoke.csv --m 3 --evicted   # D_3 -> do()
```
...
- The attacker software is **not** implemented; the compromise of `n_1` and its
  capabilities are represented only in the two-layer model (attack graph + capability
  edges). `mgmt_net` exists so the `A_i` management links are physically meaningful.
- If the iptables nft backend misbehaves under Docker Desktop, switch the images to the
  legacy backend (`update-alternatives --set iptables /usr/sbin/iptables-legacy`; see the
  Dockerfile comment).
- The static-IP scheme (`.{100+i}`) caps `m ≤ 150`; host ports `5001..5000+m` must be free.

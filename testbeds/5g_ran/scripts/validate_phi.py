...
def measured_phi(data: pd.DataFrame, weights) -> pd.Series:
    """Per-window functionality: the weighted sum over the observed columns."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate Phi on the 5G testbed.")
    ...
    # enact the mode, then measure with it pinned and the rest toggling nominally; the
    # mode's AT_i assignments define the (single) attachment phase.
    enact(mode)
    ...
    config = WindowConfig(
        ...
        settle_seconds=args.settle_seconds, seed=args.seed, phases=[reattach],
    )
    data = run_windows(config, pinned=mode)
    ranctl.apply(mode)     # clear the nominal toggles: leave exactly the enacted mode
